...
# навчання штучної нейронної мережі з 2 прихованими шарами,
...
hidden_layers_c = 2  # к-сть прихованих шарiв
...
coef_C = np.zeros(shape=(max_row_data))  # вихід - значення коефіцієнта Шезі C
...
#  W_2 - між 2-им прихованим шаром і виходом мережі
...
for iteration in range(epoch_count):
    ...
    for i in range(len(characteristics_riverbed)):
        ...
        layer_2 = np.dot(layer_1b, W_2)                 # вихідний  шар

        layer_2_e2 += np.sum((layer_2 - coef_C[i:i + 1]) ** 2)

        # зворотний хід
        layer_2_delta = (coef_C[i:i + 1] - layer_2)
        layer_1b_delta = layer_2_delta.dot(W_2.T) * logistic_deriv(layer_1b)
        layer_1a_delta = layer_1b_delta.dot(W_1_ab.T) * logistic_deriv(layer_1a)

        W_2 = W_2 + alpha * layer_1b.T.dot(layer_2_delta)
...
# W_1[input_size,hidden_size]
f = open(data_path, 'w')
for i in range (0,input_size-1):
    for j in range(0,hidden_size-1):
        f.write(str(W_1[i][j])+' ')  # запис у файл лише значень масиву через пробіл, коми і дужки не передаються
    f.write(str(W_1[i][hidden_size-1])+'\n')
for j in range(0,hidden_size-1):
   f.write(str(W_1[input_size-1][j])+' ')
f.write(str(W_1[input_size-1][hidden_size-1]))  #останній рядок записуємо поза циклом, щоб уникнути переходу на наступний рядок ('\n' - зміщення в новий рядок)
f.close()

#створюється повний шлях для відкриття файлу з врахуванням директорії, в якій запущено програму
# таке повне ім'я зберігається в data_path
data_path = join('.','Data',"weights_matrix_1_ab.txt")
data_path = abspath(data_path)

#W_1_ab[hidden_size,hidden_size]
f = open(data_path, 'w')
for i in range (0,hidden_size-1):
    for j in range(0,hidden_size-1):
        f.write(str(W_1_ab[i][j])+' ')  # запис у файл лише значень масиву через пробіл, коми і дужки не передаються
    f.write(str(W_1_ab[i][hidden_size-1])+'\n')
for j in range(0,hidden_size-1):
   f.write(str(W_1_ab[hidden_size-1][j])+' ')
f.write(str(W_1_ab[hidden_size-1][hidden_size-1]))  #останній рядок записуємо поза циклом, щоб уникнути переходу на наступний рядок ('\n' - зміщення в новий рядок)
f.close()
